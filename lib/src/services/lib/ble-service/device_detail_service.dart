...
import 'package:jump/src/services/lib/ble-service/test_scenarios/test_scenarios.dart';
...
class DeviceDetailService {
  ...
  DeviceDetailService(this._deviceRepository, this._bleManager) {
  ...
  void startAutoTest() {
    ...
    _deviceController.stream.listen((bleDevice) {
      print('got bleDevice: $bleDevice');
      bleDevice.peripheral.isConnected().then((isConnected) {
        print('The device is connected: $isConnected');
        if (!isConnected) {
